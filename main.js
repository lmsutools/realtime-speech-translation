...
ipcMain.on('model-setting-changed', (event, selectedModel) => {
  if (mainWindow && !mainWindow.isDestroyed()) { mainWindow.webContents.send('update-source-languages', selectedModel); }
});
ipcMain.on('open-typing-app', () => { createTypingAppWindow(); });
ipcMain.on('typing-app-transcript-updated', (event, fullText) => {
  if (typingAppWindow && !typingAppWindow.isDestroyed()) { typingAppWindow.webContents.send('typing-app-update-text', fullText); }
});
ipcMain.on('typing-app-recording-state-changed', (event, recording) => {
  console.log(`[Main] Recording state changed: ${recording}`);
  isRecording = recording;
  if (typingAppWindow && !typingAppWindow.isDestroyed()) { typingAppWindow.webContents.send('typing-app-recording-state', isRecording); }
});
ipcMain.on('typing-app-typing-mode-changed', (event, typingActive) => {
  runInAction(() => { appState.setTypingActive(typingActive); });
  store.set('typingActive', typingActive);
  if (mainWindow && !mainWindow.isDestroyed()) { mainWindow.webContents.send('typing-app-typing-mode-changed', typingActive); }
});
...
ipcMain.on('update-global-shortcut', (event, newShortcut) => {
  ...
  runInAction(() => { appState.setTypingAppGlobalShortcut(newShortcut); });
